...
import {addNewUser, resetInitState} from '../../redux/reducers/User';
import AppBar from '../../components/Home/AppBar';

import {verticalScale} from '../../assets/styles/scaling';
import Tab from '../../components/Tab';
import {resetCategory, updateSelectedId} from '../../redux/reducers/Category';
import {
  getDonate,
...
export default function HomeScreen({navigation}) {
  ...
  const paginationHelper = (data, myCurrpage, maxSize) => {
    const startIndex = (myCurrpage - 1) * maxSize;
    const endIndex = startIndex + maxSize;
    const temdata = category.categories.slice(startIndex, endIndex);
    if (startIndex >= category.categories.length) {
      return [];
    }
    return temdata;
  };
  ...
  function normalizeTransform() {
    const tem = {};
    donatesState.forEach(donate => {
      tem[donate.id] = donate;
    });
  ...
  useEffect(() => {
    dispatch(
      getDonate({
        byId: normalizeTransform(),
        allIds: donatesState.map(donate => donate.donationItemId),
        donations: donatesState,
      }),
    );

    setLoaded(true);
    setCurrPage(pre => pre + 1);
    const cate = paginationHelper(category.categories, currPage, 4);
    setListCategory(pre => {
      return pre.concat(cate);
    });
    setLoaded(false);
  }, []);

  console.log('curr state is ', donations);

  useEffect(() => {
    const items = donations.items?.filter(value =>
      value.categoryIds.includes(category.selectedCategoryId),
    );
    setDonationItems(items);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [category?.selectedCategoryId]);

  function handlePressDetail(id,badgeTitle) {
    console.log('press on detail', id);
  ...
  console.log(donations.selectedDonation);
  return (
    <SafeAreaView style={[globalStyle.flex, globalStyle.backgroundWhite]}>
      <ScrollView style={styles.appLayout} showsVerticalScrollIndicator={false}>
        <AppBar
          displayName={state.firstName + ' ' + state.lastName[0] + '.'}
          profileImg={state.profileUri}
        />
        <Search onSearch={handleChange} placeHolder={'Search'} />

        <Pressable style={styles.boxImageContainer}>
          <Image
        ...
        <Headline size={'md'} title={'Selected Category'} />
        <View style={styles.tabContainer}>
          <FlatList
            ...
            onEndReached={() => {
              ...
              console.log('User has scroll to half screen');
              console.log(currPage);
              const temdata = paginationHelper(
                category.categories,
                currPage,
                4,
              );
              console.log(temdata);
              setCurrPage(pre => pre + 1);
              setListCategory(pre => [...pre, ...temdata]);
              setLoaded(false);
            }}
            ...
            renderItem={item => (
              <View style={{marginHorizontal: 10}}>
                <Tab
                  ...
                  onPress={() => {
                    dispatch(updateSelectedId(item?.item?.categoryId));
                  }}
                  isActive={
                    item.item?.categoryId === category?.selectedCategoryId
                  }
                />
              </View>
            )}
        ...
        {donationItems.length > 0 && (
          <FlatList
            columnWrapperStyle={{gap: 15, columnGap: 20}} 
            ...
            renderItem={item => (
              <SingleDonationItem
                ...
                key={item.index}
                id={item.item.donationItemId}
                badgeTitle={
                  category.categories?.filter(value =>
                    item.item?.categoryIds.includes(value.categoryId),
                  )[0].name
                }
                donationTitle={item.item?.name}
                price={parseFloat(item.item?.price)}
                imgUri={item.item?.image}
              />
            )}
          />
        )}
